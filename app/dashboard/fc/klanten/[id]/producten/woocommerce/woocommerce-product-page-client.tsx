...
import { fetchWooCommerceProducts, importSelectedProducts, syncWooCommerceProducts } from "./actions"
...
interface WoocommerceProductPageClientProps {
  tenant: any
  user: TenantUser
...
export function WoocommerceProductPageClient({
  tenant,
  user,
  clientDetails,
  clientId,
  wooCommerceIntegrations,
}: WoocommerceProductPageClientProps) {
  // Set the first (oldest) integration as default
  const [selectedIntegration, setSelectedIntegration] = useState<WooCommerceIntegration | null>(
    wooCommerceIntegrations.length > 0 ? wooCommerceIntegrations[0] : null,
  )
  ...
  const getDomain = (integration: WooCommerceIntegration | null) => {
    if (!integration) return "WooCommerce Producten"

    // Try domain field first, then shop_url as fallback
    const domain = integration.domain || integration.shop_url

    if (!domain) return "WooCommerce Producten"
  ...
  const hasMultipleIntegrations = wooCommerceIntegrations.length > 1
  ...
  const loadProducts = async () => {
    ...
    try {
      const { data, error } = await fetchWooCommerceProducts(clientId, user.tenant_id, selectedIntegration.id)
  ...
  const handleSync = async () => {
    ...
    try {
      const { success, error } = await syncWooCommerceProducts(clientId, user.tenant_id, selectedIntegration.id)
  ...
  const getStatusBadge = (status: WooCommerceProduct["status"]) => {
    switch (status) {
      case "new":
        return (
  ...
  const getTypeBadge = (type: WooCommerceProduct["type"]) => {
    switch (type) {
      case "simple":
        return (
  ...
  return (
    <FCLayout user={user} tenant={tenant} searchPlaceholder="Zoek producten...">
      <div className="space-y-8 pb-16 md:pb-12">
        ...
        {/* Header met WooCommerce branding */}
        <div className="animate-in slide-in-from-top-5 duration-500 delay-100">
          <div className="bg-gradient-to-r from-purple-50 via-indigo-50 to-purple-50 rounded-2xl p-8 border shadow-lg">
                ...
                {hasMultipleIntegrations ? (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-80">
                      {wooCommerceIntegrations.map((integration) => (
                        <DropdownMenuItem
                          key={integration.id}
                  ...
                  <h1 className="text-3xl font-bold mb-2 text-slate-900">{getDomain(selectedIntegration)}</h1>
                )}
                <p className="text-slate-600 text-lg mt-2">Import products from a WooCommerce webshop</p>
                ...
                {products.length > 0 && (
                  <div className="flex gap-4 mt-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-slate-600">{statusCounts.new} nieuw</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                      <span className="text-slate-600">{statusCounts.koppelen} te koppelen</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                      <span className="text-slate-600">{statusCounts.existing} bestaand</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center flex-1">
                ...
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    className="pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 w-full"
                  />
                </div>
              ...
              <div className="flex gap-3">
                ...
                {selectedProducts.size > 0 && (
                  <Button
                    ...
                    className="relative overflow-hidden bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-700 hover:from-purple-700 hover:via-purple-800 hover:to-indigo-800 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 rounded-xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed group"
                  >
                    {/* Animated background */}
                    <div className="absolute inset-0 bg-gradient-to-r from-purple-400 via-pink-500 to-indigo-500 opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div>
              ...
              <div className="flex items-center justify-center py-12">
                <div className="text-center">
                  ...
                  <p className="text-slate-600">
                    {products.length === 0
                      ? "Er zijn geen producten beschikbaar in deze WooCommerce shop."
                    ...
                    <TableHead className="w-12 pl-6">
                      <div className="flex items-center">
                        <input
                          ...
                          className="w-4 h-4 text-purple-600 bg-white border-2 border-slate-300 rounded-md focus:ring-0 focus:outline-none transition-all duration-200 hover:border-purple-400"
                        />
                      </div>
                    </TableHead>
                ...
                <TableBody>
                  {filteredProducts.map((product, index) => (
                    <TableRow
                      ...
                      className={`
                        group transition-all duration-500 ease-out hover:bg-gradient-to-r hover:from-purple-50/50 hover:to-indigo-50/50 
                        ${product.status === "existing" ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}
                        ${
                          selectedProducts.has(product.id)
                            ? "bg-gradient-to-r from-purple-50/80 to-indigo-50/80 border-l-4 border-l-purple-500 shadow-lg transform scale-[1.01]"
                            : "hover:shadow-md"
                        }
                    ...
                    >
                        ...
                        <div className="flex items-center">
                          <div className="relative">
                            <input
                              ...
                              className="w-4 h-4 text-purple-600 bg-white border-2 border-slate-300 rounded-md focus:ring-0 focus:outline-none transition-all duration-300 hover:border-purple-400 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                            {selectedProducts.has(product.id) && (
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <div className="font-semibold text-slate-900 group-hover:text-purple-700 transition-colors duration-300">
                            {product.name}
                          </div>
                          {product.weight && (
