...
import { ShopifyProductPageClient } from "./shopify-product-page-client"
import { getAllShopifyIntegrations } from "./actions"
...
export default async function ShopifyProductsPage({ params }: PageProps) {
  ...
  // Get all Shopify integrations for this client
  const { data: shopifyIntegrations, error: integrationsError } = await getAllShopifyIntegrations(params.id, tenant.id)

  if (integrationsError || !shopifyIntegrations || shopifyIntegrations.length === 0) {
    notFound()
  }

  return (
    <ShopifyProductPageClient
      tenant={tenant}
      user={user}
