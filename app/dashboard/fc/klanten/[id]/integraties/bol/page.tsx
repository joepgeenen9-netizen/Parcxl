...
import { BolSetupClient } from "./bol-setup-client"
...
export default async function BolIntegrationPage({ params }: PageProps) {
  ...
  // Get existing bol.com integration
  const { data: existingIntegration } = await supabase
    .from("fc_client_integrations")
    .select("*")
    .eq("client_id", params.id)
    .eq("integration_type", "bol")
    .single()

  return <BolSetupClient tenant={tenant} user={user} client={client} existingIntegration={existingIntegration} />
}
